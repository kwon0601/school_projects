#include "Stack_Link.h" //int형
#include "Stack_Link2.h" //char형
...
int main()
{
	stackheadchar* stackone = createstackchar(); //스택을 만듭니다.
	stackheadchar* stacktwo = createstackchar(); //이 스택은 postfix로 만드는 과정에서 화면에 출력할 때 시스템상으로도 저장해서 나중에 계산하기 위함입니다.
	...
	stackhead* stackint = createstack(); //int형 스택을 만듭니다.
