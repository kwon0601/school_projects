...
int main()
{
	list* list_one=create_list(int_compare);
	while (select_number != 3)
	{
		...
		case 2: //Out
		{
			...
			if (remove_node(list_one, (void*)&remove_number)) printf("%d was removed.\n", remove_number);
			else printf("My List does not have %d.\n", remove_number);
			print_list(list_one);
			break;
		}
...
void print_list(list* plist)
{
	...
	reference_number = traverse_list(plist, 0, &Data);
	for (j = 1; reference_number; j++)
	{
		printf("%d", *((int*)Data));
		reference_number = traverse_list(plist, j, &Data);
		if (reference_number == 1) printf(", "); //마지막 숫자라면 ", "가 출력 안 됩니다. 
	}
	printf("\n");
